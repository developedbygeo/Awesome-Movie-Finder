...
export const getDetailTitle = (detailObj, ...queries) =>
  queries.map((query) => (query in detailObj ? detailObj[query] : null)).filter((val) => val)[0] || undefined;
...
export const getDetailGenres = (detailObj) => ('genres' in detailObj ? detailObj.genres.map((el) => el.name) : '');
